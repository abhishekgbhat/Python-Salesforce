...
def deleteProtocols(protocolToDelete):
    ...
    TRecords = sf.query("SELECT Id FROM Treament__c where Protocol__c = '%s'"%protocolToDelete)
    ...
    print (len(TRecords))," Records Deleted from Email Setup"
...
#protocolRecords = sf.query("SELECT Id,Name FROM Protocol__c ORDER BY CreatedDate DESC")
#protocolRecords = protocolRecords['records']
#print(len(protocolRecords))
#for record in protocolRecords:
     #print ""+record['Id']+"              "+record['Name']
deleteProtocols('a1w41000000QJz3')                     
